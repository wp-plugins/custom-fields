...
wpfields = {
	...
	init : function() {
		...
		$('#fields-right .add_sidebar input[type=submit]').click(function(){
			wpfields.addSidebar();
			return false;
		});
		
		$('#fields-right').children('.fields-holder-wrap').children('.sidebar-name').click(function(){
			var c = $(this).siblings('.fields-sortables'), p = $(this).parent();
		...
		$('#fields-right .del_sidebar').click(function(){
		...
		sidebars.not('#wp_inactive_fields').each(function(){
			var h = 50, H = $(this).children('.field').length;
			h = h + parseInt(H * 48, 10);
			$(this).css( 'minHeight', h + 'px' );
		});

		$('a.field-action').live('click', function(){
			var css = {}, field = $(this).closest('div.field'), inside = field.children('.field-inside'), w = parseInt( field.find('input.field-width').val(), 10 );

			if ( inside.is(':hidden') ) {
				if ( w > 250 && inside.closest('div.fields-sortables').length ) {
					css['width'] = w + 30 + 'px';
					if ( inside.closest('div.field-liquid-right').length )
		...
		sidebars.children('.field').each(function() {
			...
			if ( $('p.field-error', this).length )
				$('a.field-action', this).click();
		});
		...
		$('#available-fields').droppable({
			...
			over: function(e,ui) {
				...
				if ( ui.draggable.hasClass('ui-sortable-helper') )
					$('#removing-field').show().children('span')
					.html( ui.draggable.find('div.field-title').children('h4').html() );
			},
			out: function(e,ui) {
	...
	saveOrder : function(sb) {
		if ( sb )
			$('#' + sb).closest('div.fields-holder-wrap').find('img.ajax-feedback').css('visibility', 'visible');
	...
	save : function(field, del, animate, order) {
		var sb = field.closest('div.fields-sortables').attr('id'), data = field.find('form').serialize(), a;
		field = $(field);
	...
	addSidebar : function() {
			//alert( $('#fields-right .add_sidebar input[type=text]').val() );
			//if ( sb )
			//	$('#' + sb).closest('div.fields-holder-wrap').find('img.ajax-feedback').css('visibility', 'visible');
			...
			var a = {
				...
				savefields: $('#_wpnonce_fields').val(),
				sidebar: $('#fields-right .add_sidebar input[type=text]').val(),
				post_type: $('.post_type').val()
			};
	...
	delSidebar : function(sb) {
			...
			//if ( sb )
			//	$('#' + sb).closest('div.fields-holder-wrap').find('img.ajax-feedback').css('visibility', 'visible');
			//alert(sb.parent().children('input[type=hidden]').val());
			var a = {
	...
	appendTitle : function(field) {
		...
		if ( title = title.val() ) {
			...
			$(field).children('.field-top').children('.field-title').children()
				.children('.in-field-title').html(': ' + title);
		}
	},
...
$(document).ready(function($){ wpfields.init(); });
